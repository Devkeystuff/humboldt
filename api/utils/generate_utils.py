...
class ImageGenerator:
    ...
    def generate_distorted_map(h_map: String):
        ...
        try:
            ...
            background = cv2.resize(
                background, (800, 800), interpolation=cv2.INTER_AREA
            )
    ...
    def draw_multiple_line_text(
        image, text, font, text_color, text_start_height, vidth
    ):
        try:
            ...
            for line in lines:
                line_height = font.getsize(line)[1]
                draw.text(
                    (975, y_text), anchor="rt", text=line, font=font, fill=text_color
                )
                y_text += line_height
        except Exception as e:
            LoggingUtils.log_exception(e)
    ...
    def draw_multiple_line_text_y(text, font, text_color):
        result = None
        try:
            result = Image.new(mode="RGBA", size=(1110, 1110), color=(255, 0, 255, 0))
            ...
            for line in lines:
                line_height = font.getsize(line)[1]
                d.text(
                    (0, y_height), anchor="lt", text=line, font=font, fill=text_color
                )
                y_height += line_height
        except Exception as e:
            LoggingUtils.log_exception(e)
    ...
    def generate_design_image(
        ...
        try:
            result = Image.new(
                mode="RGBA", size=ImageGenerator.im_size, color=(255, 0, 255, 0)
            )
            d = ImageDraw.Draw(result)
            ...
            d.rectangle(
                (
                    ImageGenerator.map_size[0] + 45,
                    255,
                    ImageGenerator.map_size[0] + 46,
                    300 + ImageGenerator.map_size[1],
                ),
                fill=ImageGenerator.black,
            )
            ...
            # Add titles
            d.text(
                (ImageGenerator.im_size[0] / 2, 215),
                anchor="mb",
                align="left",
                text=location_name.upper(),
                font=ImageGenerator.title_font,
                fill=ImageGenerator.title_color,
            )
            d.text(
                (975, 1455),
                anchor="rt",
                align="center",
                text=bottom_title.upper(),
                font=ImageGenerator.name_font,
                fill=ImageGenerator.black,
            )
            ...
            ImageGenerator.draw_multiple_line_text(
                result,
                bottom_desc,
                ImageGenerator.text_font,
                ImageGenerator.black,
                2055,
                45,
            )
            result.paste(
                ImageGenerator.draw_multiple_line_text_y(
                    side_text, ImageGenerator.text_font, ImageGenerator.black
                ),
                (1200, 300),
            )
        except Exception as e:
