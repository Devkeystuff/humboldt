...
const Terrain: React.FC<ITerrainProps> = props => {
  return (
    <Canvas draggable={false} color={'black'} camera={{ zoom: 3, position: [24, 16, 16] }}>
      <pointLight castShadow intensity={1} position={[10, 5, 1]} color={'#fff'} />
      <pointLight castShadow intensity={1} position={[-10, 5, 0]} color={'#d6e09e'} />
      <Suspense fallback={<Html>Loading model...</Html>}>
        <Model elevation_img={props.elevation_img} texture_img={props.texture_img} />
      </Suspense>
